...
router.get('/user/:userId/info', authenticate, async (req: Request, res: Response) => {
    try {
  ...
  router.post('/users/info', authenticate, async (req: Request, res: Response) => {
    try {
      const { userIds } = req.body;
      
      if (!Array.isArray(userIds)) {
        res.status(400).json({ error: 'Invalid userIds format' });
        return;
    } 
      ...
      console.error(error);
      res.status(500).json({ error: 'Error fetching users info' });
    }
  });
